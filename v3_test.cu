...
__global__ void simulate_model(int *before, int *after, int N, int B) {
  ...
  block_model = (int*) malloc(N * N * sizeof(int)); 
  ...
  for (int mx = 0; mx < B; mx++) {
    for (int my = 0; my < B; my++) {
      if (i + mx < N && j + my < N) {
        ...
        neighbours[0] = get_model(block_model, i*B + mx, j*B + my + 1, N);
        neighbours[1] = get_model(block_model, i*B + mx, j*B + my - 1, N);
        neighbours[2] = get_model(block_model, i*B + mx + 1, j*B + my, N);
        neighbours[3] = get_model(block_model, i*B + mx - 1, j*B + my, N);

        after[block_index] = sign(block_model[block_index], neighbours, 4);
      }
    }
  }
}
